...
namespace SE3D
{
	...
	void ModelComponent::Render(DeferredRenderer *renderer)
	{
		...
		{
		m_Model->GetModel(i)->GetMaterial().SetParamMatrix4D(String("u_view").GetStringID(), renderer->GetCamera()->GetView());
		m_Model->GetModel(i)->GetMaterial().SetParamMatrix4D(String("u_projection").GetStringID(), renderer->GetCamera()->GetProjection());
		}
